...
"#,
    ...
    // `new Object()` allocates a JDK class outside the closed world (no object
    // layout), so the compiler-required path rejects the fixture loudly instead
    // of falling back to the evaluator's constant folding.
